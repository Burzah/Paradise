...
/obj/item/antag_spawner/nuke_ops/spawn_antag(client/C, turf/T, kind, datum/mind/user)
	...
	var/agent_number = LAZYLEN(SSticker.mode.operatives) - 1
	M.real_name = "[syndicate_name()] Operative #[agent_number]"

	set_operative_values(C, M)
...
/obj/item/antag_spawner/nuke_ops/proc/set_operative_values(client/C, mob/living/M)
	...
	SSticker.mode.operatives += M.mind
...
/obj/item/antag_spawner/nuke_ops/borg_tele/spawn_antag(client/C, turf/T, datum/mind/user)
	...
	if(!switch_roles)
		set_operative_values(C, R)
	else
		var/mob/living/L = user.current
		set_operative_values(user.current.client, R)
